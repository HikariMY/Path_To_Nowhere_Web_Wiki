...
export function CrimebrandBuildsPanel({ characterId }: { characterId: string }) {
  ...
  useEffect(() => {
    if (!characterId) { setBuilds([]); return }
  ...
  async function saveBuild() {
    if (!characterId || !form.build_name.trim()) return
    setSaving(true)
    const payload = {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-3">
        <p className="text-sm text-ptn-muted">Crimebrand Set แนะนำสำหรับตัวละครนี้</p>
      ...
      {builds.length === 0 ? (
        <div className="border border-dashed border-ptn-border rounded-lg p-10 text-center text-ptn-muted text-sm">
          ยังไม่มี Crimebrand Build สำหรับตัวละครนี้
        </div>
      ) : (
